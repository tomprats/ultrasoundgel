...
xml.rss "xmlns:itunes" => "http://www.itunes.com/dtds/podcast-1.0.dtd", version: "2.0" do
  xml.channel do
    ...
    xml.itunes :image, href: channel_image_url(@channel.uid, format: @channel.image.extension)
    ...
    @channel.episodes.published.each do |episode|
      xml.item do
        ...
        xml.itunes :image, href: episode_image_url(episode.uid, format: episode.image.extension)
        xml.enclosure(
          url: episode_audio_url(episode.uid, format: episode.audio.extension),
        ...
        xml.itunes :duration, episode.audio.duration_time
      end
    end
  end
end
